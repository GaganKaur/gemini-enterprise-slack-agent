...
def process_request(event, say, client):
                    ...
                    if adk_event.content.role == "model":
                        for part in adk_event.content.parts:
                            ...
                            if part.function_call:
                                # Provide visual feedback that a tool is being used!
                                client.chat_update(
                                    channel=channel_id,
                                    ts=message_ts,
                                    text=f"_{response_text}_\n\n*Using tool:* `{part.function_call.name}` ⚙️"
                                )
                                last_update_time = time.time()

                # --- LIVE TYPING UI EFFECT ---
                # Update Slack every 1.5 seconds if we have new text
    ...
    try:
        ...
        if final_answer:
            # Final update without the hourglass indicator
        ...
        client.chat_update(channel=channel_id, ts=message_ts, text=f"I encountered an error: `{e}`")

# --- LISTENERS ---
@slack_app.action(re.compile(".*"))
def handle_actions(ack): ack()

# CRITICAL FIX: Using ack and lazy listeners
@slack_app.message()
